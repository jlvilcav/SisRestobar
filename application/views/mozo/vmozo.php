...
<script>
  ...
  // var ahora = new Date();
  // //alert(ahora);
  // var fecha1 = moment(ahora, "DD/MM/YYYY HH:mm:ss");
  // var fecha2 = moment("27/01/2018 22:15:00", "DD/MM/YYYY HH:mm:ss");
  ...
  // var diff = fecha2.diff(fecha1, 's')/60; // Diff in hours
  // console.log(diff);
</script>
...
<header>
    <div id="topbar" style="background-color: #085981;">
        ...
        <label style="font-size: 12pt;">
            <?php echo $this->session->userdata('s_usu')." - ".$this->session->userdata('s_nomSucursal');?>
        </label>
        <!-- <label id="lblPrueba">hola</label> -->
...
<section id="croquis" ng-if="vm.mode == 'croquis' || vm.mode == 'mesa'">
    ...
    <div class="wrap-croquis">
        <div class="croquis" ng-class="{'empty': !vm.croquis}" ng-style="{transform: 'scale('+vm.scale / 100+')'}" style="transform-origin: top left;">
            ...
            <div class="mesa btn btn-app" ng-repeat="m in vm.mesas" ng-style="{left: m.x + 'px', top: m.y + 'px', width: m.ancho + 'px', height: m.alto + 'px'}" ng-class="{'busy': vm.pedidos[m.idMesa] && vm.pedidos[m.idMesa].pedido != vm.pedido.id ,'other': vm.pedidos[m.idMesa] && vm.pedidos[m.idMesa].mozo != vm.mozo,'selected': vm.pedido && vm.pedido.mesa.indexOf(m.idMesa) > -1 }" ng-click="vm.loadPedido(vm.pedidos[m.idMesa],m.idMesa)">
                <!-- <span style="font-size: 0.5em;" ng-class="{'badge bg-green': vm.pedidos[m.idMesa] && vm.pedidos[m.idMesa].pedido != vm.pedido.id}"><?php echo date('H:i');?></span> -->
                <span style="font-size: 0.5em;" data-fh="{{vm.pedidos[m.idMesa].fecHora}}" class="badge bg-green tiempoUso" ng-show="vm.pedidos[m.idMesa] && vm.pedidos[m.idMesa].pedido != vm.pedido.id">{{vm.pedidos[m.idMesa].fecHora}}</span>
                <span class="label" style="top:-12px;">{{m.nombre}}</span>
            </div>
    ...
    <div id="carta" class="box">
        <div class="box__header header">
            <div class="box-tools pull-right">
                <select class="form-control form-control-inline hidden-xs" ng-model="vm.currentCategory" ng-options="c.idCategoriaProducto as c.descripcion for c in vm.categories | orderBy:'descripcion'">
